...
export const signup_attemp = (payload, navigate) => {
  axios
    .post(
      `https://json-dummy-server-gajraj.herokuapp.com/authregister`,
      payload
    )
    .then((res) => {
...
export const signin_attemp = (payload) => (dispatch) => {
  ...
  axios
    .get(
      `https://json-dummy-server-gajraj.herokuapp.com/authregister?email=${payload.email}&password=${payload.password}`
    )
    .then((res) => {
